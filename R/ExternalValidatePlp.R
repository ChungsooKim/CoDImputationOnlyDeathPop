...
externalValidatePlp <- function(plpResult,
  ...
  for(i in 1:length(validationSchemaCdm)){
    ...
    newData <- CoDImputationOnlyDeathPop::similarPlpData(plpModel= plpResult$model, createCohorts = F, 
    ...
    if(sum(newData$population$outcomeCount>0)==0){
      ...
      if(sum(newData$population$outcomeCount>0) > 0){
      results[[i]] <- CoDImputationOnlyDeathPop::applyModel(population=newData$population, plpData = newData$plpData, 
